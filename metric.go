...
var DataType = map[string]string{
	...
	"Threads_connected":  Origin,
	"Threads_running":    Origin,
	...
	"io_thread_delay":       Origin,
	"Slave_SQL_Running":     Origin,
	"Slave_IO_Running":      Origin,
	"Seconds_Behind_Master": Origin,
}
...
func IngoreMetric(k string) bool {
	if _, ok := DataType[k]; ok {
		return true
	}
	return false
}
func AliasType(k string) string {
	/*
		某些指标存在别名,需要配置好
	*/
	str := k
	if v, ok := MetricAlias[k]; ok {
		str = v
	}
	if str != "mysqlLogin" {
...
func NewMetric(conf common.DatabaseConf, name string) *MetaData {
	return &MetaData{
		Metric:      name,
...
func (m *MetaData) SetTag(tag string) {
	m.Tags = tag
}
...
func MySQLAlive(conf common.DatabaseConf, ok bool) {
	data := NewMetric(conf, "mysqlLogin")
	data.SetValue(0)
	if ok {
		data.SetValue(1)
	}
	...
	//	conf.Hostname, conf.Port, string(msg))
}

// GetIsReadOnly get read_only variable of mysql
func GetIsReadOnly(conf common.DatabaseConf, db mysql.Conn) (int, error) {
