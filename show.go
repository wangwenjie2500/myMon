...
func ShowInnodbStatus(conf common.DatabaseConf, db mysql.Conn) ([]*MetaData, error) {
...
func ShowBinaryLogs(conf common.DatabaseConf, db mysql.Conn) ([]*MetaData, error) {
	var sum int
...
func ShowSlaveStatus(conf common.DatabaseConf, db mysql.Conn) ([]*MetaData, error) {
	// Check IsSlave
	...
	return append(data, []*MetaData{isSlaveMetric, ioDelay, slaveReadOnly}...), nil
}

// ShowOtherMetric all other metric will add in this func
func ShowOtherMetric(conf common.DatabaseConf, db mysql.Conn, metric string) (*MetaData, error) {
	var err error
	var row mysql.Row
...
func parseMySQLStatus(conf common.DatabaseConf, db mysql.Conn, sql string) ([]*MetaData, error) {
	...
	for _, row := range rows {
		...
		metric := NewMetric(conf, k)
		metric.SetValue(v)
		data = append(data, metric)
		/*
			统计tps
		*/
		if k == "Com_commit" || k == "Com_rollback" {
			translation += v
		}
		if k == "Uptime" {
			uptime += v
	...
	if uptime != 0 {
		tps := NewMetric(conf, "tps")
		tps.SetValue(translation / uptime)
		data = append(data, tps)
	}
...
func parseInnodbSection(
	conf common.DatabaseConf, row string, section string,
...
func parseInnodbStatus(conf common.DatabaseConf, rows []string) ([]*MetaData, error) {
	var section string
	longTranTime := 0
	var err error
	var data []*MetaData
